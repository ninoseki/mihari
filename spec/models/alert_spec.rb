...
RSpec.describe Mihari::Alert do
  ...
  describe ".search" do
    ...
    it do
      alerts = described_class.search(Mihari::Structs::Filters::Alert::SearchFilterWithPagination.new(tag_name: tag_name))
      expect(alerts.length).to eq(1)
    end

    it do
      alerts = described_class.search(Mihari::Structs::Filters::Alert::SearchFilterWithPagination.new(artifact_data: artifact_data))
      expect(alerts.length).to eq(1)
    end
  ...
  describe ".count" do
    ...
    it do
      count = described_class.count(Mihari::Structs::Filters::Alert::SearchFilter.new(tag_name: tag_name))
      expect(count).to eq(1)
    end

    it do
      count = described_class.count(Mihari::Structs::Filters::Alert::SearchFilter.new(artifact_data: artifact_data))
      expect(count).to eq(1)
    end
